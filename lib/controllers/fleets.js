...
module.exports = {
  ...
  addDriver: function(req, res) {
    ...
    Fleet.update({ _id: id }, { $push: { drivers: driver } }, { upsert: true }, function(err) {
      if(err) {
        res.status(500);
      } else {
        res.json(200);
  ...
  addVehicle: function(req, res)  {
    ...
    Fleet.update({ _id: id }, { $push: { vehicles: vehicle } }, { upsert: true }, function(err) {
      if(err) {
        res.status(500);
      } else {
        res.json(200);
  ...
  addTrip: function(req, res)  {
    ...
    Fleet.update({ _id: id }, { $push: { trips: trip } }, { upsert: true }, function(err) {
      if(err) {
        res.status(500);
      } else {
        res.json(200);
  ...
  addDevice: function(req, res)  {
    ...
    var id = req.params.id;
    var device = new Device(req.body.device);
    
    Fleet.update({ _id: id }, { $push: { devices: device } }, { upsert: true }, function(err) {
      if(err) {
        res.status(500);
      } else {
        res.json(200);
  ...
  getDevices: function(req, res) {
    ...
    Fleet.find({ _id: req.params.id }, { devices: 1 }, function (err, docs) {
      if (err) {
        res.status(500);
      } else {
        res.json(200, docs[0]);
